...
def insert_bot_response(response):
    ...
    # Step 1: Format bold text (**bold** → Bold)
    ...
    for match in bold_matches:
        ...
        formatted_response += response[last_index:start]  # Normal text before bold
        formatted_response += f"<bold>{match.group(1)}</bold>"  # Mark bold text
        last_index = end

    formatted_response += response[last_index:]  # Remaining text

    # Step 2: Replace '*' with '•' for bullet points (only at the start of a line)
    formatted_response = re.sub(r"(?m)^\s*\*", "•", formatted_response)

    # Step 3: Insert formatted text into the chatbox
    segments = re.split(r"(<bold>.*?</bold>)", formatted_response)

    for segment in segments:
        if segment.startswith("<bold>") and segment.endswith("</bold>"):
            chat_box.insert(tk.END, segment[6:-7], "bold")  # Insert bold text
        else:
            chat_box.insert(tk.END, segment, "bot")  # Insert normal text
...
def on_enter(e):
    send_button.config(bg="#ff7300", fg="black")
...
try:
    ...
    pass  # If the image is missing, the program still runs
